-- ─────────────────────────────────────────────────────────────
-- Mobility metrics (value = 1–10 ease rating)
-- ─────────────────────────────────────────────────────────────

INSERT INTO metrics (slug, unit, higher_is_better) VALUES
    ('deep_squat_hold',     'rating', true),
    ('pigeon_pose',         'rating', true),
    ('hip_90_90_switch',    'rating', true),
    ('couch_stretch',       'rating', true),
    ('thomas_test',         'rating', true),
    ('standing_toe_touch',  'rating', true),
    ('seated_forward_fold', 'rating', true),
    ('thoracic_rotation',   'rating', true),
    ('cat_cow_range',       'rating', true),
    ('thread_the_needle',   'rating', true),
    ('behind_back_clasp',   'rating', true),
    ('wall_slide',          'rating', true),
    ('ankle_dorsiflexion',  'rating', true);

-- ─────────────────────────────────────────────────────────────
-- Mobility category_metrics
-- ─────────────────────────────────────────────────────────────
...
FROM metrics m
WHERE m.slug IN (
    'deep_squat_hold', 'pigeon_pose', 'hip_90_90_switch',
    'couch_stretch', 'thomas_test',
    'standing_toe_touch', 'seated_forward_fold',
    'thoracic_rotation', 'cat_cow_range', 'thread_the_needle',
    'behind_back_clasp', 'wall_slide',
    'ankle_dorsiflexion'
)
ON CONFLICT DO NOTHING;

-- ─────────────────────────────────────────────────────────────
-- Mobility assessments (no equipment required)
-- ─────────────────────────────────────────────────────────────
...
INSERT INTO assessments (slug, name, description, measured_metric_id, min_level, max_level, category_id) VALUES
    ...
    ('deep_squat_hold',
     'Deep Squat Hold',
     'Lower into a deep squat and hold for 5 seconds. Rate how deep and upright you can sit on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'deep_squat_hold'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),

    ('pigeon_pose',
     'Pigeon Pose',
     'Hold a pigeon pose for 30 seconds per side. Rate how far your hip sinks toward the floor on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'pigeon_pose'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),

    ('hip_90_90_switch',
     '90/90 Hip Switch',
     'Sit in 90/90 position and switch sides 5 times without using your hands. Rate the ease and control on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'hip_90_90_switch'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),
    ...
    ('couch_stretch',
     'Couch Stretch',
     'Hold a couch stretch for 60 seconds per side. Rate how open your hip flexor feels on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'couch_stretch'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),

    ('thomas_test',
     'Thomas Test Position',
     'Lie on the edge of a surface, pull one knee to your chest and let the other leg hang. Rate how flat the hanging leg lies on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'thomas_test'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),
    ...
    ('standing_toe_touch',
     'Standing Toe Touch',
     'Stand with legs straight and reach toward your toes. Rate how close you get on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'standing_toe_touch'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),

    ('seated_forward_fold',
     'Seated Forward Fold',
     'Sit with legs straight and fold forward, reaching for your feet. Rate the ease and range on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'seated_forward_fold'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),
    ...
    ('thoracic_rotation',
     'Thoracic Rotation',
     'Sit cross-legged and rotate your upper body as far as possible to each side. Rate the range of motion on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'thoracic_rotation'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),

    ('cat_cow_range',
     'Cat-Cow Range',
     'Perform 5 slow cat-cow cycles. Rate the full range and smoothness of your spinal movement on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'cat_cow_range'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),

    ('thread_the_needle',
     'Thread the Needle',
     'From all-fours, thread one arm under your body and hold for 5 seconds per side. Rate the rotation depth on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'thread_the_needle'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),
    ...
    ('behind_back_clasp',
     ...
     'Reach one arm overhead and the other behind your back and try to clasp your hands. Rate both sides combined on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'behind_back_clasp'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),

    ('wall_slide',
     'Wall Slide',
     'Stand with your back against a wall and slide your arms up while keeping contact. Rate the ease and full range on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'wall_slide'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility')),
    ...
    ('ankle_dorsiflexion',
     'Ankle Dorsiflexion',
     'Knee-to-wall test: place your foot 10 cm from the wall and try to touch your knee to the wall without lifting your heel. Rate both sides on a scale of 1–10.',
     (SELECT id FROM metrics WHERE slug = 'ankle_dorsiflexion'),
     1, 100,
     (SELECT id FROM categories WHERE slug = 'mobility'));
...
FROM (VALUES
    ('deep_squat_hold',     jsonb_build_object('en', 'Deep Squat Hold',          'de', 'Tiefer Squat Hold')),
    ('pigeon_pose',         jsonb_build_object('en', 'Pigeon Pose',               'de', 'Pigeon Pose')),
    ('hip_90_90_switch',    jsonb_build_object('en', '90/90 Hip Switch',          'de', '90/90 Hüftwechsel')),
    ('couch_stretch',       jsonb_build_object('en', 'Couch Stretch',             'de', 'Couch Stretch')),
    ('thomas_test',         jsonb_build_object('en', 'Thomas Test Position',      'de', 'Thomas-Test-Position')),
    ('standing_toe_touch',  jsonb_build_object('en', 'Standing Toe Touch',        'de', 'Stehender Zehengriff')),
    ('seated_forward_fold', jsonb_build_object('en', 'Seated Forward Fold',       'de', 'Sitzende Vorwärtsbeuge')),
    ('thoracic_rotation',   jsonb_build_object('en', 'Thoracic Rotation',         'de', 'Thorakale Rotation')),
    ('cat_cow_range',       jsonb_build_object('en', 'Cat-Cow Range',             'de', 'Cat-Cow Bewegungsumfang')),
    ('thread_the_needle',   jsonb_build_object('en', 'Thread the Needle',         'de', 'Thread the Needle')),
    ('behind_back_clasp',   jsonb_build_object('en', 'Behind-the-back Clasp',     'de', 'Rückengriff')),
    ('wall_slide',          jsonb_build_object('en', 'Wall Slide',                'de', 'Wall Slide')),
    ('ankle_dorsiflexion',  jsonb_build_object('en', 'Ankle Dorsiflexion',        'de', 'Knöchel-Dorsalflexion'))
) AS v(slug, name)
WHERE a.slug = v.slug;
