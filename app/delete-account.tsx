            ...
            <Pressable style={styles.backdropPressable} onPress={handleClose}>
                <Pressable onPress={(e) => e.stopPropagation()}>
                    <Reanimated.View style={[styles.sheet, { backgroundColor: theme.surface }, sheetStyle]}>
                        <KeyboardAwareScrollView bounces={false} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" bottomOffset={24}>
                            <View style={[styles.content, { paddingBottom: insets.bottom + 8 }]}>
                                ...
                                <View style={styles.textBlock}>
                                    <JempText type="h2" color="#ef4444">{t('ui.delete_account_title')}</JempText>
                                    <JempText type="body-l" color={theme.textMuted} style={styles.description}>{t('ui.delete_account_message')}</JempText>
                                </View>
                                <View style={styles.field}>
                                    <JempText type="caption" color={theme.textMuted}>{t('ui.delete_account_type_prompt', { word: CONFIRM_WORD })}</JempText>
                                    <JempInput
                                        variant="outlined"
                                        value={input}
                                        ...
                                        autoCorrect={false}
                                        editable={!loading}
                                        style={[styles.confirmInput, { borderColor: confirmed ? '#ef4444' : theme.borderDivider }]}
                                    />
                                </View>
                                <TouchableOpacity
                                ...
                                >
                                    {loading ? (
                                        ...
                                        <JempText type="body-l" color="#fff">{t('ui.delete_account_confirm')}</JempText>
                                    )}
                                </TouchableOpacity>
                            </View>
...
const styles = StyleSheet.create({
    ...
    iconCircle: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(239,68,68,0.12)', alignItems: 'center', justifyContent: 'center' },
    textBlock: { gap: 8 },
    description: { lineHeight: 22 },
    field: { gap: 8 },
    confirmInput: { letterSpacing: 2, fontWeight: '600', borderWidth: 1.5 },
    deleteButton: { backgroundColor: '#ef4444', borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginTop: 4 },
